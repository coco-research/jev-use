...
### D19 - The standard deploys into a dirty repo, and says so (2026-09-20)
...
### D22 - A question is a filed object, not a message in a stream (2026-09-20)

> Numbered D22 because #27 already landed as D21 (T4 tree walk).

**The failure, measured before anything was built.** Across this machine's turn log: of 95
agent turns ending on a question, **16% were answered promptly**, 56% were followed by a
message sharing no vocabulary with the question, and 5% were never followed by anything. Of
the 31 that were an actual decision for the human, **45% were not answered promptly**.

The cause is structural, not attentional: **a question asked into a queue that does not wait
is not a question, it is a comment.** No id, no age, no owner, and nothing attached that says
what it holds up. Worse than the loss: the next queued prompt does not merely skip the
question, it **inherits an assumption nobody approved** - rule R10's failure at the workflow
level. Every incentive points at skipping the human, because an agent's cheapest action is to
keep going and a queue's whole job is to queue.

**Built:** `crates/jev-ask`, a question as an object with four fields that do the work:
`options` (a closed set, so answering is a choice not a composition), `default` (so silence
resolves and the fast path is one keystroke), `blocks` (so the cost of not answering is
visible and the queue can hold), `expires_at` (so an unanswered question is applied and
reported, never forgotten).

| Verb | Who | What it does |
| --- | --- | --- |
| `ask` | an agent | files it, with options, a default, what it blocks, and an expiry |
| `open` | the human | one ranked list, capped at 7, answerable by row number |
| `answer 1` | the human | takes the recommendation; a choice outside the set is refused and nothing is written |
| `gate` | the queue | exit 2 while something is held, naming it |
| `sweep` | a clock | applies expired defaults and says so |
| `stats` | the observer | asked / open / answered / expired, median and p90 time to answer, share that took the default |

**The store is append-only JSON Lines** (`~/.config/jev-use/decisions.jsonl`), outside every
repository, because this is machine state about several projects at once. State is a
projection of events, never a rewrite: several agents can append at once, an answer is
evidence worth keeping, and `tail -f` is a real interface. A line that does not parse is an
error naming its line number, never a silent skip.

**The gate is the part that changes behaviour.** `jev-ask gate` exits 2 while a question is
open and declares what it blocks. The queue is meant to be generated by a step that will not
emit blocked work, so a queued prompt being auto-injected stops mattering: nothing injectable
is blocked. A question with no `blocks` stays visible and ages but does not stop anything,
because stalling a lane without saying why is worse than not asking.

**A bug the first real run found.** `gate` run from a directory that is not a repository took
the cwd's name as a lane, found nothing there, and reported "queue clear" while a question
held work in another lane. A false clear is the exact failure this exists to prevent, so a
missing repo now means *every lane is checked* rather than a lane being invented.

**What it deliberately does not do:** no model call, no ranking by importance, no
notification. It cannot know what matters; it can only make what is waiting *visible, aged,
and expensive to ignore*. 17 tests, and the numbers that say whether it earns its place are
in `jev-ask stats`.
