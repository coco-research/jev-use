...
| Layer | What you get |
| --- | --- |
| **Repo Standard** | The playbook every repository carries — [`docs/repo-playbook.html`](docs/repo-playbook.html) (**Revision 5, 2026-09-20**). This repo holds the canonical copy; `scripts/standard-sync` deploys it into every repository under `products/` (and reports the directories that are not repositories, rather than writing into them). `scripts/repo-check` fails when the standard's declared checks and the checks it runs disagree. |
